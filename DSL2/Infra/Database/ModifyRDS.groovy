...
def job_name= under_folder +'/ModifyRDS'

job(job_name) {
    description 'Modify RDS'
    logRotator {
      daysToKeep -1
      numToKeep 30
    }
...
parameters {
		...
		choiceParam('action', ['modify','reboot'])
        }
steps {
			...
			'ansible-playbook --limit ${ENV_PREFIX}:local:dns  is-iaas/playbooks/modifyRDS.yml -e "envId=${envId} envName=${envName} inventory_group=atg-ora svr_create=no chargeCode=${Charge_Code} rds_instance_type=${rds_instance_type} rds_db_size_GB=${rds_db_size_GB} action=${action}" -vvvv ')
}
}
